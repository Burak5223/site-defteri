...
import { X, Building2, Phone, Mail, Save } from 'lucide-react-native';
...
const EditSiteModal: React.FC<EditSiteModalProps> = ({ visible, site, onClose, onSuccess }) => {
  const { t } = useI18n();
  ...
  const handleSubmit = async () => {
    if (!site || !formData.name || !formData.address || !formData.city) {
    ...
    setLoading(true);
    try {
      await apiClient.put(`/sites/${site.id}`, formData);
      Alert.alert(t('common.success'), 'Site başarıyla güncellendi');
      onSuccess();
      onClose();
    } catch (error: any) {
      console.error('Failed to update site:', error);
      Alert.alert(t('common.error'), error?.message || 'Site güncellenemedi');
    } finally {
      setLoading(false);
    }
  };
        ...
        <View style={styles.modalContainer}>
          ...
          <View style={styles.modalHeader}>
            <View style={styles.headerLeft}>
              ...
              <View>
                <Text style={styles.modalTitle}>Site Düzenle</Text>
          ...
          <ScrollView style={styles.modalContent}>
            ...
            {/* Site Stats (Read-only) */}
            <View style={styles.statsSection}>
              <Text style={styles.statsTitle}>Site İstatistikleri</Text>
              <View style={styles.statsGrid}>
                <View style={styles.statCard}>
                  <Text style={styles.statValue}>{site.totalApartments || 0}</Text>
                  <Text style={styles.statLabel}>Daire</Text>
                </View>
                <View style={styles.statCard}>
                  <Text style={styles.statValue}>{site.totalResidents || 0}</Text>
