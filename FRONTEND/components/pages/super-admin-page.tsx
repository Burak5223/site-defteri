...
export function SuperAdminPage({ sites, onSiteSelect, onSwitchToAdmin }: SuperAdminPageProps) {
  ...
  const loadBlocks = async (siteId: string) => {
    try {
      const response = await fetch(`http://localhost:8080/api/sites/${siteId}/blocks`, {
        headers: {
      ...
      <div className="p-4 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">{selectedBlock.name} - {selectedSite?.name}</p>
          </div>
        </div>
  ...
  // Site seçiliyse blokları göster
  if (selectedSite) {
    return (
      <div className="p-4 space-y-4">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={handleBackFromSite} className="shrink-0">
            <ArrowLeft className="w-5 h-5" />
          </Button>
          <div className="flex-1">
      ...
      <div className="p-4 space-y-4">
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-2">
            <Button variant="outline" size="sm" onClick={() => setShowAddManagerModal(true)}>
            ...
            <Button variant="outline" size="sm" onClick={() => setActiveView("managers")}>
              <Users className="w-4 h-4 mr-2" />Siteler
            </Button>
          </CardContent>
        </Card>
        ...
        <AddManagerModal 
          ...
          sites={realSites.map(site => ({ id: site.id, name: site.name, city: site.city }))}
        />
  ...
  // Managers View (Manager Addition)
  if (activeView === "managers") {
    return (
      <div className="p-4 space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold">Yönetici Ekleme</h1>
          <Button size="sm" onClick={() => setShowAddManagerModal(true)}>
            <Plus className="w-4 h-4 mr-2" />Yönetici Ekle
          </Button>
        </div>
        ...
        <Card className="bg-gradient-to-br from-primary/10 to-primary/5 border-primary/20">
          <CardContent className="p-4 text-center">
            ...
            <h3 className="font-semibold mb-2">Yeni Yönetici Ekle</h3>
            <p className="text-sm text-muted-foreground mb-4">
              Sitelerinize yeni yönetici atayın ve yetkilendirin
            </p>
            <Button onClick={() => setShowAddManagerModal(true)} className="w-full">
              <Plus className="w-4 h-4 mr-2" />
              Yönetici Ekleme Modalını Aç
            </Button>
          </CardContent>
        </Card>
        ...
        <div className="text-sm text-muted-foreground mb-2">
          Mevcut Siteler ({filteredSites.length})
        </div>
