...
print("=== 1. SAKIN KULLANICI BULMA ===")
cursor.execute("SELECT id, email, full_name FROM users WHERE email = 'sakin@test'")
...
if user:
    ...
    print("\n=== 2. KULLANICININ DAİRESİ ===")
    cursor.execute("""
        SELECT rh.apartment_id, a.apartment_number, b.name as block_name
        FROM residency_history rh
        JOIN apartments a ON rh.apartment_id = a.id
        JOIN blocks b ON a.block_id = b.id
        WHERE rh.user_id = %s AND rh.end_date IS NULL
    """, (user_id,))
    residency = cursor.fetchone()
    
    if residency:
        print(f"✓ Daire: {residency['block_name']} - {residency['apartment_number']}")
        print(f"  Apartment ID: {residency['apartment_id']}")
        apartment_id = residency['apartment_id']
    else:
        print("❌ Aktif daire kaydı bulunamadı!")
        apartment_id = None
    
    print("\n=== 3. KULLANICIYA ATANAN AİDATLAR ===")
    cursor.execute("""
    ...
    print("\n=== 5. SON OLUŞTURULAN AİDATLAR ===")
    cursor.execute("""
        SELECT d.id, d.title, d.amount, d.user_id, d.apartment_id,
               u.email, a.apartment_number, b.name as block_name
        FROM dues d
